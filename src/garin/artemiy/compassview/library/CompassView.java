...
/**
 * Author: Artemiy Garin Date: 26.11.13
 */
public class CompassView extends ImageView {
		...
		if (CompassUtility.isDeviceCompatible(context)) {
			if (!(context instanceof FragmentActivity && context instanceof CompassSensorsActivity))
				throw new RuntimeException(
						"Your activity must extends from CompassSensorsActivity");
		} else
			setVisibility(GONE);
	}

	public void initializeCompass(Location userLocation,
	...
	public void initializeCompass(Location userLocation,
			Location objectLocation, int drawableResource, TextView tvHeading) {
		if (CompassUtility.isDeviceCompatible(context)) {
			this.userLocation = userLocation;
			this.objectLocation = objectLocation;
			this.drawableResource = drawableResource;
	...
	private void startRotation() {
		...
		float rotation = Math.round(bearTo - azimuth);
		if (rotation < 0)
			rotation = rotation + DEGREES_360;

		float deltaRotation = Math.abs(rotation - lastRotation);
		if (deltaRotation <= MIN_DELTA) {
			rotation = lastRotation;
		}

		if (tvHeading != null)
			tvHeading.setText(Float.toString(rotation));
	...
	private void rotateImageView(ImageView compassView, int drawable, float currentRotate) {
		...
		if (directionBitmap == null) {
			maxDelta = 0.0f;
			directionBitmap = BitmapFactory.decodeResource(getResources(),
					drawable);
			...
			compassView.setImageDrawable(new BitmapDrawable(getResources(),
					directionBitmap));
			...
			RotateAnimation rotateAnimation = new RotateAnimation(lastRotation,
					currentRotate, Animation.RELATIVE_TO_SELF, CENTER,
	...
	private class CustomAnimationListener implements
